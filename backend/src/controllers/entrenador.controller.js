...
const CreateEntrenador = async(req, res) => {
    ...
    try {
      ...
      WHERE ID_ENTRENADOR=$1 AND COD_DISCIPLINA=$2`, [id_usuario, cod_disciplina]
        );
        ...
        if (existingPersona.rowCount === 0) {
            ...
            const disciplina = await pool.query(
                "INSERT INTO ENTRENADOR_DISCIPLINA (ID_ENTRENADOR,COD_DISCIPLINA) VALUES ($1,$2) RETURNING *", [id_usuario, cod_disciplina]
            );
            ...
            for (const horario of horarios) {
                await pool.query(
                    "INSERT INTO Disciplina_Horario (cod_disciplina, id_horario) VALUES ($1, $2)", [idNuevaDisciplina, horario.id_horario]
                );
            }
    ...
    try {
      ...
      DISCIPLINA.COD AS COD_DISCIPLINA,
        ...
        WHERE
          ...
          AND ENTRENADOR.ID_USUARIO = $1`, [ID_ENTRENADOR]
        );
...
const updateEntrenador = async(req, res, next) => {
    try {
        ...
        const existingEntrenador = await pool.query(
            "SELECT * FROM ENTRENADOR_DISCIPLINA WHERE ID_ENTRENADOR = $1", [id_usuario]
        );
        ...
        const result = await pool.query(
            "UPDATE ENTRENADOR_DISCIPLINA SET COD_DISCIPLINA = $1 WHERE ID_ENTRENADOR = $2 RETURNING *", [cod_disciplina, id_usuario]
        );

        // Eliminar horarios existentes asociados a la disciplina
        await pool.query(
            "DELETE FROM Disciplina_Horario WHERE cod_disciplina = $1", [cod_disciplina]
        );

        // Insertar nuevos horarios asociados a la disciplina
        for (const horario of horarios) {
            await pool.query(
                "INSERT INTO Disciplina_Horario (cod_disciplina, id_horario) VALUES ($1, $2)", [cod_disciplina, horario.id_horario]
            );
        }
