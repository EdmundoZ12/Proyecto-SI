  ...
  try {
    ...
    if (existingPersona.rowCount === 0) {
      ...
      if (existingUser.rowCount === 0) {
        ...
        const accion = `creo El Usaurio con ID = ${Id_Persona}`;
...
const getUsers = async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT Persona.*,Usuario.Username, Usuario.password ,Usuario.Id_Rol,Usuario.Direccion,Usuario.Activo FROM Usuario,Persona Where Persona.Id=Usuario.Id_Persona"
    );
...
const updatePersonAndUser = async (req, res) => {
  ...
  try {
    ...
    const personaUpdateResult = await client.query(personaUpdateQuery, [
      nombre,
      apellido,
      telefono,
      fecha_nacimiento,
