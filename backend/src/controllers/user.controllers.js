...
const CreateUsuario = async(req, res) => {
    ...
    try {
        ...
        const existingPersona = await pool.query(
            "SELECT * FROM Persona WHERE CI= $1", [ci]
        );
        ...
        if (existingPersona.rowCount === 0) {
            ...
            if (existingUser.rowCount === 0) {
                ...
                const persona = await pool.query(
                    "INSERT INTO Persona (Nombre, Apellido, CI, Telefono, Fecha_Nacimiento) VALUES ($1, $2,$3,$4,$5) RETURNING Id", [nombre, apellido, ci, telefono, fecha_nacimiento]
                );
                const Id_Persona = persona.rows[0].id;
                const passwordhash = await bcrypt.hash(password, 10);
                const result = await pool.query(
                    "INSERT INTO Usuario (Id_Persona,Username, Password, Id_Rol, Direccion, Activo) VALUES ($1, $2,$3,$4,$5,$6) RETURNING  Id_Persona,Username,Direccion,Activo", [Id_Persona, username, passwordhash, id_rol, direccion, activo]
                );
                ...
                const { tokenB } = req.cookies;
                const accion = `creo usuario con ID = ${result.rows[0]}`;
                if (tokenB) {
                    console.log("entro");
                    const decodedToken = jwt.verify(tokenB, TOKEN_SECRET);
...
const getUser = async(req, res) => {
    ...
    try {
        const result = await pool.query(
            "SELECT Persona.*,Usuario.Username, Usuario.password ,Usuario.Id_Rol,Usuario.Direccion,Usuario.Activo FROM Usuario,Persona Where Persona.Id=Usuario.Id_Persona and Usuario.Username = $1", [username]
        );
