...
export default function FacturaDemo() {
  ...
  const { Factura, createFactura, getFacturas, getFacturass } =
    useFactura();
  const {
    createProducto,
    updateProducto,
  ...
  const [FacturaDialog, setEntradaDialog] = useState(false);
  ...
  useEffect(() => {
    getFacturass(); //base falsa
    //getFacturas  //base real
    getProveedores();
    getProductos();
    user.id;
  }, []);
  ...
  const saveNota = () => {
    ...
    if (entrada.cod > 0) {
      ...
      setEntradaDialog(false);
      setEntrada(emptyEntrada);
      setProductosFactura([]); // Limpia los productos después de guardar la nota
    }
  };
  // ...
  ...
  const imprimir=()=>{
      ...
      for (let i = 0; i < productosFactura.length; i++) { 
        var dato=[`${productosFactura[i].nombre}`,`${productosFactura[i].cantidad}`,`${productosFactura[i].precio}`,`${productosFactura[i].cantidad*productosFactura[i].precio}`]
        data.push(dato)
        monto=monto+(productosFactura[i].cantidad*productosFactura[i].precio)
      }
