...
export default function Nota_EntradasDemo() {
  ...
  const { nota_Entrada, createNota_Entrada, getNotaa } =
    useNota_Entrada();
  const {
    createProducto,
    updateProducto,
  ...
  const imprimir=()=>{
      ...
      var monto=0
      for (let i = 0; i < productosEntrada.length; i++) {
        var dato=[`${productosEntrada[i].nombre}`,`${productosEntrada[i].cantidad}`,`${productosEntrada[i].precio}`,`${productosEntrada[i].cantidad*productosEntrada[i].precio}`]
        data.push(dato)
        monto=monto+(productosEntrada[i].cantidad*productosEntrada[i].precio)
      }
