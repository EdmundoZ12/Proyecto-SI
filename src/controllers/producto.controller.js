...
const getProducto = async(req, res) => {
    ...
    try {
        ...
        const rol = await pool.query(`select producto.cod,producto.nombre,producto.descripcion,producto.precio,categoria.nombre as categoria
                                        from producto
                                        join categoria on categoria.id=producto.cod
                                        where producto.cod = $1`, [id]);
...
const createProducto = async(req, res) => {
    ...
    try {
        ...
        try {
            ...
            const insertResult = await client.query("INSERT INTO producto (nombre, descripcion, precio, id_categoria) VALUES ($1, $2, $3, $4) RETURNING *", [nombre, descripcion, precio, id_categoria]);
...
const updateProducto = async(req, res) => {
    ...
    try {
        ...
        const existingProvider = await pool.query(
            "SELECT * FROM producto WHERE nombre = $1 AND cod <> $2", [nombre, id]
        );
...
const deleteProducto = async(req, res) => {
    ...
    try {
        ...
        const existingRol = await pool.query(
            `SELECT * FROM ${tabla} WHERE cod = $1`, [id]
        );
